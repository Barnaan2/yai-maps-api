...
exports.verify = catchAsync(async(req,res,next)=>{
    ...
    try{
        if(!(req.headers.apikey && req.headers.secretkey)){
         next(new AppError('Please provide your api key or read the documentation for detail',400))
        }
        ...
        if(user){
            const correct = await user.findSecretKey(req.headers.secretkey,user.secretKey)
            console.log(correct)
        if(correct){
            req.user = user
            next()
