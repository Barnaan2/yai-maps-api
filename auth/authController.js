...
// Login controller.
// my password for sms getway platform
// twilio auth token  a92b8dd98c653fbc6ec52c0047fc44f0
// twilio account sid ACee0233afa82b9aa11eede6deb8d66ef3
exports.login = catchAsync(async (req, res, next) => {
  try {
    if (!(req.body.password && req.body.phoneNumber)) {
      next(
        new AppError(
          "Both phone number{phoneNumber} and password{password} should be provided to login !",
          400
        )
      );
    }
    ...
    if (user) {
      ...
      if (correct) {
        ...
        next(
          new AppError(
            "User phoneNumber doesn't exits or the password is not correct!",
            400
          )
        );
      }
